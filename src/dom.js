...
const globalFooter = () => {
        ...
        const bgSwitch = createElement('button', ['btn'], 'bg-switch');
                bgSwitch.innerHTML = bgIcon;
                bgSwitch.addEventListener('click', changeBg);


        const githubButton = createElement('button', ['btn'], 'github');
        ...
        const scale = state.scale.getCurrent() === 'Fahrenheit' ? 'F' : 'C';
        const scaleBtn = createElement('button', ['btn'], 'scale', `${scale}\u00B0`);
...
function resolveFutureWeatherElements(futureData){
        for (let i = 1; i <= 5; i++){
                ...
                const minValue = createElement('span', ['future-day-low-value', 'scalable'], null,  String(`${data.minTemp}\u00B0`));
                const minTemp = createElement('p', ['future-day-low', 'future-day-text'], null);
                minTemp.append(minLabel, minValue);

                const maxLabel = createElement('span', ['future-day-label', 'high'], null, 'High');
                const maxValue = createElement('span', ['future-day-high-value', 'scalable'], null, String(`${data.maxTemp}\u00B0`))
                const maxTemp = createElement('p', ['future-day-high', 'future-day-text']);
                maxTemp.append(maxLabel, maxValue);
...
function resolveBodyElements(current, today){
        ...
        temps.forEach(temp => {
                ...
                const display = createElement('p', ['scalable'], `${id}-display`, `${String(today[references.dataNames[id]])}\u00B0`);
...
const getScalableElements = () => {
        return document.querySelectorAll('.scalable');
}