...
export function initDom () {
                console.log(state);
                main.append(homePage(), globalFooter());
                return null;
}

const homePage = () => {
        const con = createElement('div', ['home-el'], 'home-container');
        const head = createElement('h2', ['home-el'], null, "Zehno's Weather");
...
export function renderOptions(options){
        try {
                ...
                if (!options.length) {
                        throw new Error('No options found. Try again following this format: Chicago IL');
                }
                for (const option of options) {
                        console.log("formatted option", option);
                        const op = createElement('p', ['home-el'], 'location-found', String(option.formatted));
                        console.log('new element:', op);
